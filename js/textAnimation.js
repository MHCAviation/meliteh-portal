document.addEventListener("DOMContentLoaded", function () {
  function initializeTypingEffect() {
    ...
    setTimeout(function () {
      ...
      function eraseText(index = rightJobElement.innerText.length - 1) {
        if (index >= 0) {
          rightJobElement.innerHTML =
            rightJobElement.innerText.slice(0, index) +
  ...
  // Call initializeTypingEffect on DOMContentLoaded
  initializeTypingEffect();

  // Add an event listener for the custom 'page:load' event
  document.addEventListener("page:load", initializeTypingEffect);
});
